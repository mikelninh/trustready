...
import {evaluateLocalPromotionGates,loadPinnedAssuranceFromEnv,trustAnchorsDistinct,verifiedClaim} from '../core/legal-assurance-evidence.mjs'
...
const regressionNames=[
  ...
  'local pre-audit can never self-assert independent assurance or real mandate readiness',
  ...
  'production transport rejects self-asserted HSM and test transport cannot become production transport',
  'production transport rejects self-asserted HSM signers while test transport remains explicitly test-branded',
  ...
  'expired preparation and failed fresh authorization block before https request',
  'real node https.request uses only the prepared one-shot agent socket',
  'final synchronous reauthorization is the last gate before request submission',
  'async final clock or async authorization is denied before any provider request',
  'production metadata identity is native-only and custom metadata providers stay test-only',
  'effective firewall policy rules without match fail closed',
  'malformed individual classic and policy firewall rules fail closed',
  'malformed VPC-SC policy fields and any extra restricted API fail closed',
  'network posture rejects a hardened decoy workload that is not the executing instance',
  'production network collector rejects injected Cloud API clients by construction',
  'production DLP and WORM adapters reject injectable test backends',
  ...
  'VPC Service Controls restricts all services used by the mandate shadow path without escape-policy blocks',
  'ECDSA HSM-style envelope verification matches Cloud KMS SHA-256 semantics',
  'Sensitive Data Protection test scanner pins config and fails closed on malformed response PII or outage',
  'network enforcement is bound to exact workload effective policy layers and IPv4-only posture',
  'network test collector fetches exact runtime workload and policy views without becoming production collector',
  'WORM test store requires locked retention and immutable generation receipt',
  'infrastructure qualification rejects test and self-asserted adapters instead of issuing synthetic candidate',
  'production mandate pipeline rejects test transport even when NODE_ENV is test',
  'production mandate pipeline rejects fake or test DLP scanner before egress',
  'production mandate pipeline rejects test network collector before egress',
  'production mandate pipeline rejects fake or test WORM store before egress',
  'production mandate pipeline rejects self-asserted hardware-backed HSM signers',
  'production mandate pipeline rejects caller-asserted rooted key store',
  'production adapter gate executes before provider, DLP, network or WORM I/O',
  'production refuses caller-assembled trust roots and accepts offline-root-signed keyring',
  'root fingerprint mismatch and keyring tampering fail closed',
...
const independentRequired=['independent_pentest','independent_legal_privacy_review','independent_evidence_verification','organizational_independence']
const independentObserved=independentRequired.filter(k=>verifiedClaim(independentAssurance,k,{levels:['E4'],now}))
const organizationalIndependenceClaim=verifiedClaim(independentAssurance,'organizational_independence',{levels:['E4'],now})
...
if(!independentEvidenceComplete)report.blockers.push('separately signed independent penetration/legal/privacy/evidence/organizational-independence evidence incomplete')
...
if(independentEvidenceComplete&&distinctTrustAnchors&&!organizationalIndependenceClaim)report.blockers.push('externally evidenced organizational independence claim missing')
report.blockers.push('final independent assurance verdict must be issued outside the local pre-audit process')
