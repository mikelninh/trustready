...
export function evaluatePilotReadiness({ audit_report, env = process.env }) {
  ...
  const sendReleaseApproved = audit_report?.verdict?.human_approved_send_ready === true

  const realEvidenceSatisfied = engineering && preAudit && realRequested && liveReady && independentlyAssured
  const humanApprovedSendReady = realEvidenceSatisfied && sendRequested && sendReleaseApproved
  const syntheticPilotReady = engineering && preAudit && !realRequested && !sendRequested
  const realMandatePilotReady = realEvidenceSatisfied && (!sendRequested || humanApprovedSendReady)
  ...
  if (sendRequested && !sendReleaseApproved) blockers.push('HUMAN_APPROVED_SEND_NOT_SEPARATELY_RELEASED')
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!audit.report) {
    console.error(JSON.stringify({ schema: 'trustready-bao-pilot-readiness-v1', synthetic_pilot_ready: false, real_mandate_pilot_ready: false, human_approved_send_ready: false, blockers: ['AUDIT_REPORT_UNAVAILABLE'] }, null, 2))
    process.exit(1)
  }
