...
function normalizeStrictJson(value, state = { seen: new WeakSet(), nodes: 0 }, depth = 0, path = '$') {
  ...
  try {
    if (Array.isArray(value)) {
      ...
      for (const name of Object.keys(descriptors)) { if (name === 'length') continue; if (!/^(0|[1-9]\d*)$/.test(name)) throw new TypeError(`${path}: non-index array property denied`); const index = Number(name); if (!Number.isSafeInteger(index) || index < 0 || index >= length) throw new TypeError(`${path}: invalid array index denied`) }
      for (let index = 0; index < length; index += 1) { const descriptor = descriptors[String(index)]; if (!descriptor || !descriptor.enumerable || !Object.hasOwn(descriptor, 'value')) throw new TypeError(`${path}[${index}]: sparse/accessor array entry denied`); out[index] = normalizeStrictJson(descriptor.value, state, depth + 1, `${path}[${index}]`) }
      return Object.freeze(out)
    }
    const prototype = Object.getPrototypeOf(value); if (prototype !== Object.prototype && prototype !== null) throw new TypeError(`${path}: custom object prototype denied`); if (Object.getOwnPropertySymbols(value).length) throw new TypeError(`${path}: symbol properties denied`)
    const descriptors = Object.getOwnPropertyDescriptors(value), out = Object.create(null)
    for (const key of Object.keys(descriptors).sort()) { const descriptor = descriptors[key]; if (!descriptor.enumerable || !Object.hasOwn(descriptor, 'value')) throw new TypeError(`${path}.${key}: accessor/non-enumerable property denied`); Object.defineProperty(out, key, { value: normalizeStrictJson(descriptor.value, state, depth + 1, `${path}.${key}`), enumerable: true, writable: false, configurable: false }) }
    return Object.freeze(out)
  } finally { state.seen.delete(value) }
}

async function hsmPosture(signer, label) { if (!isProductionGoogleCloudHsmSigner(signer)) throw new Error(`${label} must be a production Google Cloud HSM signer`); const posture = await signer.posture(); if (!posture?.ready || posture.protection_level !== 'HSM' || posture.algorithm !== 'EC_SIGN_P256_SHA256' || !posture.key_version_name || !cryptoKeyIdentity(posture.key_version_name)) throw new Error(`${label} HSM posture invalid`); return posture }
...
async function providerToken(provider) { if (typeof provider !== 'function') throw new Error('provider token source required'); const value = await provider(); if (typeof value !== 'string' || value.length < 16 || /[\r\n]/.test(value)) throw new Error('provider token unavailable'); return value }
...
function productionAdaptersReady({ key_store, dlp_scanner, dlp_signer, network_collector, egress_signer, restricted_transport, evidence_signer, worm_store }) {
  ...
  if (!dlp_scanner.project_id || dlp_scanner.project_id !== network_collector.project_id) return { ready: false, code: 'DLP_PROJECT_MISMATCH', reason: 'production DLP project must equal the executing gateway project before mandate data inspection' }
  ...
  for (const [label, signer] of [['DLP', dlp_signer], ['egress', egress_signer], ['evidence', evidence_signer]]) if (!isProductionGoogleCloudHsmSigner(signer)) return { ready: false, code: 'PRODUCTION_HSM_SIGNER_REQUIRED', reason: `${label} signer must be a production Google Cloud HSM signer` }
  return { ready: true }
}
...
export async function runGcpMandateShadowPipeline(input = {}) {
  ...
  const adapters = productionAdaptersReady({ key_store, dlp_scanner, dlp_signer, network_collector, egress_signer, restricted_transport, evidence_signer, worm_store }); if (!adapters.ready) return notReady(adapters.code, adapters.reason)
  let pipelineRequest; try { pipelineRequest = normalizeStrictJson(request) } catch (error) { return notReady('PAYLOAD_SERIALIZATION_DENIED', error.message) }
  ...
  let postures
  try {
    const [dlpPosture, egressPosture, evidencePosture, networkPosture] = await Promise.all([hsmPosture(dlp_signer, 'DLP signer'), hsmPosture(egress_signer, 'egress signer'), hsmPosture(evidence_signer, 'evidence signer'), restrictedTransportPosture(restricted_transport)])
  ...
  const hsmKeys = Object.values(postures).map((p) => p.key_version_name), cryptoKeys = Object.values(postures).map((p) => cryptoKeyIdentity(p.key_version_name))
  ...
  if (dlp.scan?.scanner_project_id !== network_collector.project_id || dlp.scan?.scanner_location !== PRODUCTION_DLP_LOCATION) return notReady('DLP_DEPLOYMENT_PROOF_MISMATCH', 'DLP attestation deployment differs from protected EU gateway deployment')
  ...
  if (enforcement.posture?.project_id !== dlp_scanner.project_id) return notReady('NETWORK_PROJECT_PROOF_MISMATCH', 'network posture project differs from DLP project')
  ...
  let vertex; try { vertex = buildVertexProposalRequest({ payload: pipelineRequest.payload, use_case: pipelineRequest.use_case }) } catch (error) { return notReady('MODEL_REQUEST_DENIED', error.message) }
  ...
  let accessToken; try { accessToken = await providerToken(provider_token_provider) } catch (error) { cancelPreparedGoogleApiRequest(prepared.prepared); return notReady('PROVIDER_AUTH_DENIED', error.message) }
  let finalDecision = initialDecision
  const providerResponse = await sendPreparedGoogleApiRequest({
    transport: restricted_transport, prepared: prepared.prepared, headers: { authorization: `Bearer ${accessToken}` }, clock: PRODUCTION_CLOCK,
    before_send: (sendNow) => {
  ...
  let signedManifest; try { signedManifest = await signEnvelopeWithSigner({ body: manifest, signer: evidence_signer, purpose: 'evidence_manifest' }) } catch (error) { return notReady('EVIDENCE_SIGNING_FAILED', error.message) }
