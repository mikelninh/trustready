...
const networkCollector = { async collect() { return { ready: true, provider: 'gcp-vpc-service-controls', deny_by_default: true, only_restricted_google_apis: true, restricted_vip: '199.36.153.4/30', perimeter_name: 'legal', protected_resource: 'projects/123', protected_network: 'https://www.googleapis.com/compute/v1/projects/p/global/networks/legal', deny_rule: 'deny-all', allow_rule: 'restricted-only' } } }

function tlsFactory({ remote='199.36.153.4', authorised=true }={}) { return () => { const socket=new EventEmitter();socket.authorized=authorised;socket.remoteAddress=remote;socket.alpnProtocol='http/1.1';socket.getPeerCertificate=()=>({raw:Buffer.from('pipeline-cert')});socket.setTimeout=()=>socket;socket.destroy=()=>{};queueMicrotask(()=>socket.emit('secureConnect'));return socket } }
