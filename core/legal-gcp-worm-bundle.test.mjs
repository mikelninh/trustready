...
function storeWith({ failPath = null, missingRetention = false } = {}) {
  ...
  return {
    ...
    async append({ object_name, bytes }) {
      ...
      return {
        stored: true, bucket: 'evidence', project_number: '123', object_name, generation: String(writes.length),
        content_hash: `sha256:${sha256(Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes))}`,
        retention_expiration_time: missingRetention ? null : '2026-10-03T12:00:00Z',
      }
    },
  }
}
...
test('verified artifacts are written before COMMITTED manifest marker', async () => {
  ...
  const result = await commitEvidenceBundleToWorm({ store, signed_manifest: signedManifest(), key_store: keyStore, artifacts, bundle_id: 'bundle-1', now: NOW })
...
test('tampered artifact is rejected before any immutable write', async () => {
  ...
  const result = await commitEvidenceBundleToWorm({ store, signed_manifest: signedManifest(), key_store: keyStore, artifacts: tampered, bundle_id: 'bundle-2', now: NOW })
...
test('partial artifact write never creates commit marker', async () => {
  ...
  const result = await commitEvidenceBundleToWorm({ store, signed_manifest: signedManifest(), key_store: keyStore, artifacts, bundle_id: 'bundle-3', now: NOW })
...
test('missing retention receipt fails bundle rather than pretending immutability', async () => {
  ...
  const result = await commitEvidenceBundleToWorm({ store, signed_manifest: signedManifest(), key_store: keyStore, artifacts, bundle_id: 'bundle-4', now: NOW })
...
test('unsafe traversal path cannot enter immutable evidence namespace', async () => {
  ...
  const store = storeWith()
  const result = await commitEvidenceBundleToWorm({ store, signed_manifest: signedManifest(unsafeArtifacts), key_store: keyStore, artifacts: unsafeArtifacts, bundle_id: 'bundle-5', now: NOW })
