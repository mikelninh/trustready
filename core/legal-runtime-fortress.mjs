...
export async function executeApprovedAction({capability,identity_assertion,matter_authorization,key_store,tenant_id,matter_id,action,payload,policy_version,runtime_state,deployment_mode='shadow',replay_store,action_handler,now=new Date()}){
  ...
  if(deployment_mode==='shadow'&&HIGH_RISK_ACTIONS.has(action))return{executed:false,code:'SHADOW_LOCK',reason:'high-risk action disabled in shadow mode'}
  const id=verifyIdentityAssertion({assertion:identity_assertion,key_store,now})
  if(!id.valid||id.principal.tenant_id!==tenant_id)return{executed:false,code:'IDENTITY_DENIED',reason:id.reason||'tenant mismatch'}
  const matter=authorizeMatter({principal:id.principal,tenant_id,matter_id,operation:'execute_action',zone:'MANDATE'})
  if(!matter.allowed)return{executed:false,code:'MATTER_DENIED',reason:matter.reason}
  const schema=validateActionPayload(action,payload)
  if(!schema.valid)return{executed:false,code:'ACTION_SCHEMA_DENIED',reason:schema.reason}
  if(runtime_state.production===true){
    const fresh=verifyMatterAuthorization({authorization:matter_authorization,key_store,expected:{subject:id.principal.subject,tenant_id,session_id:id.principal.session_id,matter_id,resource_version:payload.resource_version},operation:'execute_action',now})
    if(!fresh.valid)return{executed:false,code:'FRESH_MATTER_AUTH_REQUIRED',reason:fresh.reason}
  }
  const v=verifyEnvelope({envelope:capability,key_store,purpose:'action_approval',now})
  if(!v.valid)return{executed:false,code:'APPROVAL_DENIED',reason:v.reason}
  const b=v.body
  if(b.schema!=='trustready-action-approval-v2'||parseTime(b.expires_at)<=now.getTime())return{executed:false,code:'APPROVAL_DENIED',reason:'approval expired/unsupported'}
  const expected={actor_id:id.principal.subject,actor_session_id:id.principal.session_id,tenant_id,matter_id,action,payload_hash:schema.payload_hash,resource_version:payload.resource_version,policy_version}
  for(const[k,val]of Object.entries(expected))if(b[k]!==val)return{executed:false,code:'APPROVAL_MISMATCH',reason:`${k} mismatch`}
  if(!replay_store||replay_store.durable!==true||replay_store.atomic!==true||typeof replay_store.claim!=='function')return{executed:false,code:'REPLAY_STORE_REQUIRED',reason:'durable atomic replay store required'}
  const claimed=await replay_store.claim({idempotency_key:b.capability_id,nonce:b.nonce,expires_at:b.expires_at})
  if(claimed!==true)return{executed:false,code:'REPLAY_DENIED',reason:'capability already consumed or replay store unavailable'}
  if(typeof action_handler!=='function')return{executed:false,code:'HANDLER_REQUIRED',reason:'typed action handler required; capability consumed fail-safe'}
  const result=await action_handler({actor:id.principal.subject,tenant_id,matter_id,action,payload,capability_id:b.capability_id})
  return{executed:true,code:'EXECUTED',capability_id:b.capability_id,result}
}
