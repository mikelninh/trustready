...
import { buildProspectSecuritySignalV2 } from './engine-v2.mjs'
...
const report={version:'trustready-prospect-v2-batch/v2',generatedAt:new Date().toISOString(),targets:targets.length,completed:completed.length,failed:results.length-completed.length,distribution,reviewQueue:queue.map(x=>({repository:x.repository,classification:x.signal.classification,priority:x.signal.priority,revision:x.signal.revision,flows:x.signal.evidence.selectorFlows.slice(0,3)})),metrics,precision:metrics.proofGap,results}
