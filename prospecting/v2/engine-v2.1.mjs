...
function mappingSelectorFlows(snapshot) {
  ...
  for(const [path,content] of Object.entries(snapshot?.files||{})) {
    ...
    for(let i=0;i<rows.length;i++) {
      ...
      const mapRe=new RegExp(`(?:self\\.)?(?:tool_mapping|tool_map|tool_fn_map|tools|handlers|actions|functions|available_functions|AVAILABLE_FUNCTIONS|available)\\s*(?:\\[\\s*${v}\\s*\\]|\\.get\\s*\\(\\s*${v}\\s*\\))`,'i')
...
export function buildProspectSecuritySignalV21(snapshot,{repository=null,segment=null,description=null}={}) {
  ...
  return {
    ...
    evidence:{...(base.evidence||{}),selectorFlows:selectorFlows.slice(0,20),consequences,consequenceCoupled:hasConsequentialSurface && selectorFlows.length>0},
