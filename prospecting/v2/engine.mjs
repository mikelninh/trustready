...
function selectorCandidates(content) {
  ...
  for(const re of assignmentPatterns){
    let m
    while((m=re.exec(content))){ out.push({kind:'assignment',variable:cleanVar(m[1]),expr:m[2],line:lineNo(content,m.index)}) }
  }

  // Python: `for tool in message.tool_calls:` then `session.call_tool(tool.name, ...)`.
  const pyLoop=/\b(?:async\s+for|for)\s+([A-Za-z_][\w]*)\s+in\s+(?:iter_to_aiter\s*\(\s*)?([^\n:)]*(?:tool_calls|toolCalls)[^\n:)]*)\)?\s*:/gi
  let p
  while((p=pyLoop.exec(content))){
    const item=cleanVar(p[1])
    out.push({kind:'loop_item',variable:item,expr:`${item}.name`,line:lineNo(content,p.index),source:p[2].trim()})
  }

  // JS/TS: `for (const toolCall of response.tool_calls) { ... }`.
  const jsLoop=/for\s*\(\s*(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s+of\s+([^)]*(?:tool_calls|toolCalls)[^)]*)\)/gi
  let j
  while((j=jsLoop.exec(content))){
    const item=cleanVar(j[1])
    out.push({kind:'loop_item',variable:item,expr:`${item}.name`,line:lineNo(content,j.index),source:j[2].trim()})
  }
...
function selectorControlsSink(content, candidate) {
  const escaped = candidate.variable ? reEscape(candidate.variable) : null
  const expr = reEscape(candidate.expr)
  const sinkPatterns=[]
  if(candidate.kind==='loop_item' && escaped){
    sinkPatterns.push(
      new RegExp(`(?:session\\.)?(?:call_tool|callTool)\\s*\\(\\s*${escaped}(?:\\.function)?\\.(?:name|tool_name|toolName)\\b`,'i'),
      new RegExp(`getattr\\s*\\([^\\n]{0,220},\\s*${escaped}(?:\\.function)?\\.(?:name|tool_name|toolName)`,'i'),
      new RegExp(`(?:registry|tools?|handlers?|actions?|functions?)\\s*\\[\\s*${escaped}(?:\\.function)?\\.(?:name|tool_name|toolName)\\s*\\][\\s\\S]{0,300}(?:\\(|invoke|execute|run|call)`,'i'),
    )
  }
  if(escaped && candidate.kind!=='loop_item'){
    sinkPatterns.push(
...
export function analyzeFile(path, content, repoMeta={}) {
  ...
  for(const candidate of selectors){
    ...
    // Ignore impossible backwards matches where a sink before a loop/assignment was accidentally paired.
    if(sink.line < candidate.line) continue
    const localControl=localControlBetween(content,candidate.line,sink.line)
    flows.push({selectorKind:candidate.kind,selectorLine:candidate.line,selectorExpr:candidate.expr.slice(0,240),selectorVariable:candidate.variable,selectorSource:candidate.source||null,sinkLine:sink.line,sinkSnippet:sink.snippet,localAuthorityControl:localControl})
  }
  const controls=[]
  rows(content).forEach((r,i)=>{ if(CONTROL_RE.test(r)) controls.push(i+1); CONTROL_RE.lastIndex=0 })
  return {path,role,selectorFlows:flows.slice(0,12),effects:effectEvidence(content),controls:controls.slice(0,12)}
}
...
export function buildProspectSecuritySignalV2(snapshot,{repository=null,segment=null,description=null}={}) {
  ...
  const frameworkLike=production.some(f=>f.role==='framework') || /framework|sdk|library|protocol|orchestrat|calibration framework/i.test(String(segment||''))
