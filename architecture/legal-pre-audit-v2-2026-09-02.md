...
14. execute deterministic Legal Egress authorization;
15. obtain provider credential;
16. synchronously execute the complete fresh Legal Egress authorization as the final pre-send gate;
17. with no async boundary, send exact prepared bytes over the one-shot attested socket;
18. validate model response as non-executable proposal;
19. build, HSM-sign and verify evidence manifest against actual artifacts;
20. commit artifacts immutably to the pre-qualified WORM destination, manifest last;
21. only then may the engineering pipeline return `CANDIDATE`.
